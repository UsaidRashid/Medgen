...
export default function AddBrandedMedicine() {
  ...
  const handleAlternativeSaltChange = (altIndex, saltIndex, event) => {
    ...
    const updatedAlternatives = formData.alternatives.map((alternative, i) => {
      if (i === altIndex) {
        const updatedSalts = alternative.salt.map((salt, j) =>
  ...
  const deleteAlternativeSaltField = (altIndex, saltIndex) => {
    const updatedAlternatives = formData.alternatives.map((alternative, i) => {
      if (i === altIndex) {
        const updatedSalts = alternative.salt.filter((_, j) => j !== saltIndex);
        return { ...alternative, salt: updatedSalts };
      }
      return alternative;
    });
  ...
  const deleteAlternativeField = (index) => {
    const updatedAlternatives = formData.alternatives.filter(
      (_, i) => i !== index
    );
    setFormData({ ...formData, alternatives: updatedAlternatives });
  };
          ...
          <div className="overflow-auto" style={{ maxHeight: "70vh" }}>
            <form className="form-group p-2 my-4" onSubmit={handleSubmit}>
              ...
              <div className="mb-3">
                ...
                {formData.alternatives.map((alternative, altIndex) => (
                  <div key={altIndex} className="mb-3">
                    <h6>Alternative {altIndex + 1}</h6>
                    <div className="mb-3 px-3">
                      <label
